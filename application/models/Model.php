...
class Model extends CI_Model
{
	...
	public function notifAbsensi()
	{
		$Query = $this->db->query("SELECT  *, COUNT(id) AS jml_update_temp FROM temp_log_absen");
		return $Query->result_array();
	}
	...
	public function ViewOvertimePerWeek($WhereValue1, $WhereValue2)
	{
		$Query = $this->db->query("	SELECT A.id_pegawai, A.nik,	A.nama,	B.attlog, B.tanggal, MIN( B.waktu ) AS jam_datang, MAX( B.waktu ) AS jam_pulang, B.waktu 
									FROM
										tb_pegawai A
										LEFT JOIN log_absen B ON B.pin = A.nik 
										AND (
	...
	public function ViewNotifAbsensiSelected($WhereValue1)
	{
		$Query = $this->db->query("	SELECT
									...
									B.attlog,
									B.tanggal,
									MIN( B.waktu ) AS jam_datang,
									MAX( B.waktu ) AS jam_pulang,
									B.waktu 
								FROM
									tb_pegawai A
									LEFT JOIN log_absen B ON B.pin = A.nik
