    ...
    <link href="<?=base_url()?>assets/plugins/datatables/dataTables.bootstrap.css" rel="stylesheet" type="text/css" />
      <table class="table table-striped table-bordered DataTablePegawaiOffice" >
          <thead>
            <tr>
              <td>No</td>
              <td>Nama</td>
          ...
          <tbody>
            <tr>
            <?php $no = 0 ; foreach ($office as $key => $vaPegawai) { ?>
            <?php //include 'detail.pegawai.php'; ?>
              ...
              <a href="#" class="btn btn-success btn-flat edit-record" onclick="GetDataModalOffice(<?=$vaPegawai['id_pegawai']?>);"  data-id="<?=$vaPegawai['id_pegawai']?>">
        ...
        <div class="modal fade bs-example-modal-lg" id="office" tabindex="-1" role="dialog" aria-hidden="true">
...
<!-- DATA TABES SCRIPT -->
    ...
    <script>
         function GetDataModalOffice(id){
          $("#office").modal('show');
          $.ajax({
            type: "GET",
