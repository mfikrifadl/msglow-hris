...
header("Content-Disposition: attachment; filename=BNI Sy Payroll - PT. Global Insight Utama - ".$cBulan."-".date("Y")." .xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,1,'NOPEG',$header);  // Set Nama kolom
$worksheet1->set_column(0,1,8); // Set lebar kolom

$worksheet1->write_string(0,2,'NAMAPEG',$header);  // Set Nama kolom
$worksheet1->set_column(0,2,35); // Set lebar kolom 

$worksheet1->write_string(0,3,'NOREK',$header);  // Set Nama kolom
$worksheet1->set_column(0,3,14); // Set lebar kolom

$worksheet1->write_string(0,4,' JMLGAJI ',$header);  // Set Nama kolom
...
$worksheet1->write_string($row+1,1, '');
$worksheet1->write_string($row+2,1, 'BIAYA TRANSAKSI (JML REK X RP 1500)');
$worksheet1->write_string($row+3,1, 'TOTAL');
$worksheet1->write_string($row+4,1, '');

$worksheet1->write_string($row+5,1, 'Bekasi, '.date("d").' '.$cBulan.' '.date("Y").' ');
$worksheet1->write_string($row+6,1, 'PT. Global Insight Utama');
$worksheet1->write_string($row+7,1, '');
$worksheet1->write_string($row+8,1, '');
$worksheet1->write_string($row+9,1, '');
$worksheet1->write_string($row+10,1, 'Dewi M. Ratnasari');
$worksheet1->write_string($row+11,1, 'HRD  Payroll');
