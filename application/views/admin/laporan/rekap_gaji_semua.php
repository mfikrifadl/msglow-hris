...
header("Content-Disposition: attachment; filename=RekapGaji Bulan - ".$cBulan." Tahun - ".$tahun.".xls" );
header("Expires: 0");
...
$worksheet1->write_string(2,4,'Nama Bank',$header);  // Set Nama kolom
...
$worksheet1->write_string(0,0,'Rekap Gaji Operator Bulan '.$cBulan.' Tahun '.$tahun.' ',$judul);  // Set Nama kolom
...
foreach ($jos->result_array() as $key ) {
    ...
    $worksheet1->write_string($row,4,  $cNamaBank ,$content);
    ...
    $worksheet1->write_string($row,24,  round($key['bonus_shift_bulan_sebelumnya'],-3) ,$content);
    ...
    $worksheet1->write_string($row+1,17,  ($nGajiPokok) ,$content);
    $worksheet1->write_string($row+1,18,  ($nUangMakanHariBiasa) ,$content);
    $worksheet1->write_string($row+1,19,  ($nUangMakanHariLibur) ,$content);
    $worksheet1->write_string($row+1,20,  ($nUangMakanLongshiftBiasa) ,$content);
    $worksheet1->write_string($row+1,21,  ($nUangMakanLongshiftLibur) ,$content);
    $worksheet1->write_string($row+1,22,  ($nUangLaporan) ,$content);
    $worksheet1->write_string($row+1,23,  ($nUangKesehatan) ,$content);
    $worksheet1->write_string($row+1,24,  ($nBonusSebelumnya) ,$content);
    $worksheet1->write_string($row+1,25,  ($nInsentif1) ,$content);
    
    $worksheet1->write_string($row+1,26,  ($nInsentif2) ,$content);
    $worksheet1->write_string($row+1,27,  ($nInsentif3) ,$content);
    $worksheet1->write_string($row+1,28,  ($nInsentif6) ,$content);
    $worksheet1->write_string($row+1,29,  ($nInsentif4) ,$content);
    $worksheet1->write_string($row+1,30,  ($nInsentif5) ,$content);
    $worksheet1->write_string($row+1,31,  ($nInsentif7) ,$content);
    

    $worksheet1->write_string($row+1,32,  ($nPinjaman) ,$content);
    $worksheet1->write_string($row+1,33,  ($nDenda) ,$content);
    $worksheet1->write_string($row+1,34,  ($nLainLain) ,$content);
    $worksheet1->write_string($row+1,35,  ($nTotal) ,$content);
