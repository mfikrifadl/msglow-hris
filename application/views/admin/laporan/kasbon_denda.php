...
header("Content-Disposition: attachment; filename=Kasbon Denda Bulan - ".$cBulan." Tahun - ".$tahun."- Spv - ".$nama_spv." .xls" );
header("Expires: 0");
...
$worksheet1->write_string(2,4,'Denda',$header);  // Set Nama kolom
$worksheet1->set_column(0,4,20); // Set lebar kolo

$worksheet1->write_string(2,5,'Keterangan Denda',$header);  // Set Nama kolom
...
$worksheet1->write_string(0,0,'Kasbon Denda Operator '.$cBulan.' Tahun '.$tahun.'',$judul);  // Set Nama kolom
$worksheet1->merge_cells(0, 0, 1, 5);
...
$jos = $this->db->query("CALL kasbon_denda('".$bulan."','".$tahun."')");
$no = 0 ;
foreach ($jos->result_array() as $key) {
    ...
    $worksheet1->write_string($row,4,  $key['denda'] ,$content);
    $worksheet1->write_string($row,5,  $key['denda_keterangan'] ,$content);
