...
header("Content-Disposition: attachment; filename=Insentif Operator(".$cPembayaran.") - PT. Global Insight Utama - ".$cBulan."-".date("Y")." .xls" );
header("Expires: 0");
...
$worksheet1->write_string(2,11,'24',$header);  // Set Nama kolom
$worksheet1->set_column(0,11,4); // Set lebar kolom

$worksheet1->write_string(2,12,'25',$header);  // Set Nama kolom
$worksheet1->set_column(0,12,4); // Set lebar kolom

$worksheet1->write_string(2,13,'26',$header);  // Set Nama kolom
$worksheet1->set_column(0,13,4); // Set lebar kolom

$worksheet1->write_string(2,14,'27',$header);  // Set Nama kolom
$worksheet1->set_column(0,14,4); // Set lebar kolom

$worksheet1->write_string(2,15,'28',$header);  // Set Nama kolom
$worksheet1->set_column(0,15,4); // Set lebar kolom

$worksheet1->write_string(2,16,'29',$header);  // Set Nama kolom
$worksheet1->set_column(0,16,4); // Set lebar kolom

$worksheet1->write_string(2,17,'30',$header);  // Set Nama kolom
$worksheet1->set_column(0,17,4); // Set lebar kolom


$worksheet1->write_string(2,18,'31',$header);  // Set Nama kolom
...
$worksheet1->write_string(0,0,'Insentif  Operator Bulan  Tahun '.$tahun.' - PEMBAYARAN '.$cPembayaran.' ',$judul);  // Set Nama kolom
...
$jos = $this->db->query("SELECT * FROM v_insentif WHERE bulan = '".$bulan."' and tahun = '".$tahun."' and id_pembayaran = '".$bayar."' ORDER BY nama_outlet");
$no         = 0 ;
foreach ($jos->result_array() as $key) {
    ...
    $worksheet1->write_string($row,3,  $pembayaran ,$content);
    ...
    $worksheet1->write_string($row,11,  $key['tanggal_24'] ,$content);
    $worksheet1->write_string($row,12,  $key['tanggal_25'] ,$content);
    $worksheet1->write_string($row,13,  $key['tanggal_26'] ,$content);
    $worksheet1->write_string($row,14,  $key['tanggal_27'] ,$content);
    $worksheet1->write_string($row,15,  $key['tanggal_28'] ,$content);
    $worksheet1->write_string($row,16,  $key['tanggal_29'] ,$content);
    $worksheet1->write_string($row,17,  $key['tanggal_30'] ,$content);
    $worksheet1->write_string($row,18,  $key['tanggal_31'] ,$content);  
    $row++;
}
$workbook->close();
