...
header("Content-Disposition: attachment; filename=Laporan Mutasi Karyawan PT.GIU ".$tgl_awal." s/d ".$tgl_akhir."- Dari ".$nama_area_awal." Ke ".$nama_area_akhir." .xls" );
header("Expires: 0");
...
$worksheet1->write_string(3,1,'Per - '.$tgl_awal.' s/d '.$tgl_akhir.' - Dari '.$nama_area_awal.' Ke '.$nama_area_akhir.'',$title);  // Set Nama kolom
...
$jos = $this->db->query("SELECT * FROM v_mutasi WHERE tanggal >= '".$tgl_awal_query."' and 
                                                 tanggal <= '".$tgl_akhir_query."' and 
                                                 IdAreaLama = '".$id_area_awal."' and 
                                                 IdAreaBaru = '".$id_area_akhir."'");   
