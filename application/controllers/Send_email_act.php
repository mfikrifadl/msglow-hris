...
class Send_email_act extends CI_Controller
{
            ...
            if ($next_test == "SELAMAT ANDA TELAH LOLOS SEMUA TES") {
                ...
                if ($this->email->send()) {
                    ...
                    if ($status_tes == "TIDAKLOLOS") {
                        ...
                        $data = array(
                            'status_email_tes_kesehatan' => "DELIVERED"
                        );
                    }
                    ...
                    if ($status_tes == "TIDAKLOLOS") {
                        ...
                        $data = array(
                            'status_email_tes_kesehatan' => "NOT SENT"
                        );
                    }
                    $this->model->Update('recruitment', 'id_recruitment', $id, $data);
                }
    ...
    public function send_email_phl($controller_name = "", $id = "")
    {

        if ($id == "") {
        } else {

            $db = $this->model->ViewWhere('recruitment_phl', 'id_recruitment_phl', $id);
            $nama_R = "";
            $email_R = "";
            $status_tes = "";
            $next_test = "";
            ...
            foreach ($db as $key => $vaDataR) {
                ...
                $email_R = $vaDataR['email'];
                $tahap_r = strtoupper($vaDataR['tahap_r']);
                $posisi = strtoupper($vaDataR['divisi']);

                if ($controller_name == "administrasi") {
                    $status_tes = strtoupper($vaDataR['administrasi']);
                }
                if ($controller_name == "wawancara_hr") {
                    $status_tes = strtoupper($vaDataR['wawancara_hr']);
                }
                if ($controller_name == "interview_user_1") {
                    $status_tes = strtoupper($vaDataR['interview_user_1']);
                }
                if ($controller_name == "tes_kesehatan_phl") {
                    $status_tes = strtoupper($vaDataR['tes_kesehatan_phl']);
                }
                if ($controller_name == "peserta_diterima") {
                    $status_tes = strtoupper($vaDataR['peserta_diterima']);
                }


                $tgl_wawancara = $vaDataR['tanggal_wawancara'];
                $tgl_wawancara_hr = $vaDataR['tgl_wawancara_hr'];
                $tgl_interview_u1 = $vaDataR['tgl_interview_user_1'];
                $tgl_tes_kesehatan = $vaDataR['tgl_tes_kesehatan_phl'];

                //========================== FUNGSI NEXT TEST CALON PELAMAR ==============================================

                if ($tahap_r == "TEST ADMINISTRASI") {
                    $next_test = "WAWANCARA HR";
                    $tgl_test = $tgl_wawancara;
                } elseif ($tahap_r == "WAWANCARA HR") {
                    $next_test = "INTERVIEW USER 1";
                    $tgl_test = $tgl_wawancara_hr;
                } elseif ($tahap_r == "INTERVIEW USER 1") {
                    $next_test = "TES KESEHATAN";
                    $tgl_test = $tgl_interview_u1;
                } elseif ($tahap_r == "TES KESEHATAN") {
            ...
            if ($next_test == "SELAMAT ANDA TELAH LOLOS SEMUA TES") {
                ...
                if ($this->email->send()) {
                    ...
                    if ($status_tes == "TIDAKLOLOS") {
                        ...
                        $data = array(
                            'status_email_tes_kesehatan_phl' => "DELIVERED"
                        );
                    }
                    $this->model->Update('recruitment_phl', 'id_recruitment_phl', $id, $data);
                } else {
                    $data = array();
                    if ($status_tes == "TIDAKLOLOS") {
                        $data = array(
                        ...
                        $data = array(
                            'status_email_tes_kesehatan_phl' => "NOT SENT"
                        );
                    }
                    $this->model->Update('recruitment_phl', 'id_recruitment_phl', $id, $data);
                }
                ...
                if ($this->email->send()) {
                    if ($controller_name == "administrasi") {
                        ...
                        if ($status_tes == "TIDAKLOLOS") {
                            ...
                            $data = array(
                                'status_email_wawancara_hr' => "DELIVERED"
                            );
                        }
                        $this->model->Update('recruitment_phl', 'id_recruitment_phl', $id, $data);
                    } elseif ($controller_name == "interview_user_1") {
                        ...
                        if ($status_tes == "TIDAKLOLOS") {
                            ...
                            $data = array(
                                'status_email_interview_u1' => "DELIVERED"
                            );
                        }
                        $this->model->Update('recruitment_phl', 'id_recruitment_phl', $id, $data);
                    ...
                    if ($controller_name == "administrasi") {
                        ...
                        if ($status_tes == "TIDAKLOLOS") {
                            ...
                            $data = array(
                                'status_email_wawancara_hr' => "NOT SENT"
                            );
                        }
                        $this->model->Update('recruitment_phl', 'id_recruitment_phl', $id, $data);
                    } elseif ($controller_name == "interview_user_1") {
                        ...
                        if ($status_tes == "TIDAKLOLOS") {
                            ...
                            $data = array(
                                'status_email_interview_u1' => "NOT SENT"
                            );
                        }
                        $this->model->Update('recruitment_phl', 'id_recruitment_phl', $id, $data);
                    } elseif ($controller_name == "tes_kesehatan_phl") {
            ...
            redirect(site_url('recruitment_phl/' . $controller_name . '/S'));
        }
    }
}
